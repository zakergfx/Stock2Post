...
class tempMakesView(APIView):
    authentication_classes = []
    permission_classes = [AllowAny]

    def get(self, request):

        data = [{"id": 0, "name": "Audi", "models": 
                 [{"id": 0, "name": "A1", "vehicleType": "C"}, {"id": 1, "name": "A2", "vehicleType": "C"}]}, 
                 {"id": 1, "name": "BMW", "models": 
                  [{"id": 2, "name": "Serie 1", "vehicleType": "C"}, {"id": 3, "name": "X1", "vehicleType": "C"}]}]
        return Response(data, status=200)

class MakesView(APIView):
    authentication_classes = []
    permission_classes = [AllowAny]

    def get(self, request):
        makes = models.Make.objects.all()
...
class ModelsView(APIView):
    ...
    def get(self, request):
        ...
        make = models.Model.objects.filter(fk_make__name__iexact=make)
        serializer = serializers.ModelSerializer(make, many=True)
    ...
    def post(self, request):
        ...
        if step == 1:
            ...
            if user:
                ...
                user.set_password("Fmei&W2@GAvHKru4ZfR3")
                user.save()
...
class InstagramLinkView(APIView):
    def get(self, request, *ars, **kwargs):
        ...
        body = {
            "code": code,
            "client_id": 3041512645997798,
            "client_secret": "0d52c9d19b58af9049f79d7390822155",
        ...
        url = f"https://graph.instagram.com/access_token?grant_type=ig_exchange_token&client_secret=0d52c9d19b58af9049f79d7390822155&access_token={access_token}"
