import requests, time, json, os, hashlib, cv2, random, re, shutil
...
PROXIES = [
               {"http": "http://14a79a1db1505:8e022c9119@95.214.182.162:12323"},

               ]

def testing():
    url = "https://app.loicktest.be/api/tempmakes/"
    ...
    for make in makes:
        makeObject, created = models.Make.objects.get_or_create(as_id=make["id"], name=make["name"])
        for model in make["models"]:
            modelObject, created = models.Model.objects.get_or_create(as_id=model["id"], name=model["name"], vehicleType=model["vehicleType"], fk_make=makeObject)
...
def createVideo(ad):
    ...
    if (ad.fk_dealer.name == "Lovacar"):
        images_url = [images_url[0] for _ in range(10)]
...
def createSoldPicture(ad):
    data = requests.get(ad.mainPicture, proxies=random.choice(PROXIES)).content
...
def createAdDict(url):
    ...
    ad["price"] = tools.formatPrice(soup.find(class_="PriceInfo_price__XU0aF").find(text=True, recursive=False).strip())
    ...
    try:
        ...
        ad["mainPicture"] = "https://static.vecteezy.com/system/resources/previews/008/296/135/non_2x/sports-car-icon-isolated-on-white-background-free-vector.jpg"
...
def uploadPictures(ad, dealer):
    ...
    if ad.pictures is not None:
        pictures = ad.pictures.split("-----")
    else:
        pictures = [ad.mainPicture]
...
def reuploadAds(FBweeks, IGWeeks, dealer):
    ...
    for ad in ads:
        ...
        if dealer.fbToken and dealer.fk_settings.FBcreateOldCarPost and isTimestampOlderThan(FBweeks, ad.date):
            createPost(ad, msg, dealer)
            ad.date = int(time.time())+random.randint(0, 43200)
...
def updateAllPrices(dealer):
    ...
    while pagesRemaining:
        ...
        response = requests.get(url, proxies=random.choice(PROXIES))
        soup = BeautifulSoup(response.text, "html.parser")
        pageAds = soup.find_all(class_="dp-listing-item__wrapper")
        # dp-link dp-listing-item-title-wrapper
        index+=1
        if len(pageAds) > 0:
            ads += pageAds
        else:
        ...
        adsDict = {}
    for ad in ads:
        price = ad.find(class_="dp-listing-item__price")
        if price is None:
            price = ad.find(class_="dp-listing-item__superdeal-highlight-price-span")
    ...
    for ad in ads:
        ad.lastPrice = ad.price
        ad.price = adsDict[ad.url.split("-")[-1]]
        ad.save()
...
def postDiscountAds(dealer):
    ...
    for ad in ads:
        if ad.price < ad.lastPrice:
            ...
            ad.isModified = False
            ad.date = int(time.time())+random.randint(0, 43200) # new
            ad.save()
...
def createTestPost(dealer, scenario):
    if scenario == 0:
        ...
        ad = models.Ad.objects.filter(fk_dealer=dealer).last()
        msg = f"""💲💲💲{boldText("PROMOTION EXCEPTIONNELLE")} 💲💲💲\n\nLe prix de ce modèle de {boldText(ad.model)} est maintenant à {boldText(formatNumber(int(0.9*ad.price))+" €")} au lieu de {boldText(formatNumber(ad.price)+" €")} \n\n🛣️ {boldText("Premiere immatriculation")} : {ad.release}\n🌍 {boldText("Kilometrage")} : {formatNumber(ad.km)} km\n⛽ {boldText("Carburant")} : {ad.fuel}\n🛞 {boldText("Transmission")} : {"Automatique" if ad.isAutomatic else "Manuelle"}\n🚀 {boldText("Puissance")} : {ad.kw} kw ({ad.ch} ch)\n\n{boldText("Telephone")} : {ad.fk_dealer.phone}\n{boldText("Mail")} : {ad.fk_dealer.mail}\n\n{boldText("Pour + d'infos")} : {ad.url}"""
        createDiscountPicture(ad)
...
def scheduledTask():
    ...
    for dealer in dealers:
        if dealer.isInit:
            ...
            adsSold, adsToAdd = getAdsChanges(dealer.name)
